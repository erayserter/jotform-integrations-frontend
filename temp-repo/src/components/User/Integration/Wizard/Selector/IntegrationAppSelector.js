...
const IntegrationAppSelector = (props) => {
  ...
  const authHandler = async (event) => {
    ...
    if (res.content.responseCode === 200) {
      ...
      props.onAuthenticate(
        [selectedApp.id, selectedApp.action, selectedApp.key],
        props.type,
        res.content.content
      );
    } else {
  ...
  useEffect(() => {
    ...
    if (props.datas[props.type][0] !== null) {
      if (props.isValid) {
        setButtonText("Authenticated");
        setSelectedApp((prev) => {
          return { ...prev, key: props.datas[props.type][2] };
        });
        setIsAppSelectorVisible(true);
      ...
      setSelectedApp((prev) => {
        return {
          ...prev,
          id: props.datas[props.type][0],
          action: props.datas[props.type][1],
        };
      });
    }
